...
where:
    ...
    -s,  --start-sniffing    start sniffing
        Options:
          ...
          -o, --output-dir   output directory, where tcpdump files will be generated(it will create folder by name tcpdump-data where pcap files will be stored
        Example:
...
function start_sniffing() {
  ...
  pod_list=`kubectl get pods -n $NAMESPACE | grep -i $POD | awk '{print $1}'`
  ...
  for pod in $pod_list;do
    ...
    then
      ...
      kubectl sniff -p $pod -n $NAMESPACE -o $pcap_dir/$pod.pcap > /dev/null 2>&1 &
...
function stop_sniffing() {
  ...
  ksniff_pod_list=`kubectl get pods -n $NAMESPACE | grep -i ksniff | awk '{print $1}'`
  for kpod in $ksniff_pod_list;do
    kubectl delete pod $kpod -n $NAMESPACE
  done
